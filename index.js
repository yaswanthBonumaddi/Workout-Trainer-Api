...
mongoose.connect("mongodb+srv://yaswanthbonumaddi9:Apple123@cluster0.8daxbt3.mongodb.net/?retryWrites=true&w=majority").then(()=>console.log("DB Connected")).catch((err)=>console.log(err)) 
...
app.post('/register',async (req,res)=>{
    try{
        ...
        if (exist){
            return res.status(400).send({status:"ok",msg:'User Already Registered'})
        }
        if (username==="" || email==="" || mobile==="" || password==="" ){
            return res.status(400).send({status:"notok",msg:'All fields are required'});
          }
        if(password != confirmpassword){
            return res.status(400).send({status:"notok",msg:'Passsword & Confirm Password are not matched'});
        }
        ...
        await newUser.save();
        return res.status(200).send({status:"ok",msg:'User Registered successfully'});
    }
    catch(err){
        console.log(err);
...
app.post('/login', async (req,res)=>{
    try{
        ...
        if (exist.password != password){
            return res.status(400).send({status:"notok",msg:"Password Invalied"})
        }
        let payload ={
            user:{
                id:exist.id
...
app.get('/myprofile',middleware,async(req,res)=>{
    try{
        let user = await fitnessusers.findById(req.user.id);
...
app.get('/exercises/:bodyPart',middleware, async(req,res)=> {
    try{
        ...
        let allexercises = await gymexercises.find();
        let filteredExercises = allexercises.filter(exercise => exercise.bodyPart.toString() === bodyPart.toString())
...
app.get('/exercises/exercise/:id',middleware, async(req,res)=> {
    try{
        const {id} = req.params
        let allexercises = await gymexercises.find();
        let filteredExercises = allexercises.filter(exercise => exercise.id.toString() === id.toString())
        return res.json(filteredExercises);
    }
    catch(err){
        console.log(err);
